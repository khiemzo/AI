...
temp_img = None  # Ảnh tạm khi vẽ ROI
...
def process_image():
    ...
    num_circles = 0
    circle_coords = []  # Tạo danh sách lưu tọa độ các hình tròn
    if circles is not None:
        ...
        for i in circles[0, :]:
            ...
            if roi is None or (roi[0] <= i[0] <= roi[2] and roi[1] <= i[1] <= roi[3]):
                ...
                circle_coords.append((i[0], i[1], i[2]))  # Thêm tọa độ và bán kính hình tròn vào danh sách
    ...
    # Cập nhật kết quả vào danh sách để in khi nhấn nút "Print Results"
    global current_results
    current_results = (param1, param2, min_radius, max_radius, num_circles, circle_coords)
...
# In các thông số, tọa độ ROI và tọa độ các hình tròn vào terminal
def print_roi_info():
    # Gộp 4 tham số thành dạng (x, y, a, b)
    params = f"({current_results[0]}, {current_results[1]}, {current_results[2]}, {current_results[3]})"

    # Thông tin ROI
    roi_info = f"ROI: ({roi[0]}, {roi[1]}) to ({roi[2]}, {roi[3]})" if roi else "ROI: Not selected"

    # Số lượng hình tròn
    circle_info = f"Circles Detected: {current_results[4]}"

    # In tọa độ hình tròn
    circles_output = "\nCircle Coordinates: "
    if current_results[5]:
        for i, (x, y, r) in enumerate(current_results[5], 1):
            circles_output += f"\n{str(i)}. (x: {x}, y: {y}, r: {r})"
    else:
        circles_output += "None"
...
def load_image_from_url():
    ...
    try:
        ...
        roi_info_label.config(text=f"Error loading image: {e}")
...
root = Tk()
root.title("Circle Detection with ROI and Parameters Display")
...
def create_slider_with_buttons(root, label_text, row, from_, to_, initial_value, slider_command):
    ...
    slider = Scale(root, from_=from_, to_=to_, orient=HORIZONTAL, length=200,
...
param1_slider = create_slider_with_buttons(controls_frame, "Canny Upper Threshold", 2, 0, 300, 100,
                                           lambda _: process_image())
param2_slider = create_slider_with_buttons(controls_frame, "Accumulator Threshold", 3, 0, 100, 30,
                                           lambda _: process_image())
...
# Hiển thị thông số ROI và tham số
roi_info_label = Label(controls_frame, text="", bg="#e6e6e6", font=("Arial", 10), justify=LEFT, anchor=W)
roi_info_label.grid(row=7, column=0, columnspan=4, pady=10)

# Nút in kết quả ra console
print_button = Button(controls_frame, text="Print Results", command=print_roi_info,
                      bg="#28a745", fg="white", font=("Arial", 12, "bold"),
                      relief=RAISED, padx=5, pady=5)
print_button.grid(row=8, column=0, columnspan=4, pady=10)
